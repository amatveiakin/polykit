...
ClusterCoRanks cluster_co_ranks(const SpaceT& space) {
  const int space_rank = space_mapping_ranks(space, std::identity{}, [](const auto& expr) {
    return keep_non_weakly_separated(expr);
  }).kernel();
  const int kernel_rank = space_mapping_ranks(space, std::identity{}, [](const auto& expr) {
