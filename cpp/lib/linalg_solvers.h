...
SpaceT minimal_containing_subspace(const SpaceT& haystack, const SpaceT& needle) {
  CHECK(space_contains(haystack, needle, std::identity{}));
  auto ret = haystack;
  int idx = 0;
  while (idx < ret.size()) {
    auto new_ret = ret;
    new_ret.erase(new_ret.begin() + idx);
    if (space_contains(new_ret, needle, std::identity{})) {
...
ExprT find_equation(ExprT expr, const std::vector<ExprT>& space, const std::vector<int>& coeff_candidates = {1, -1}) {
  ...
  CHECK(space_contains(space, {expr}, std::identity{}));
  ...
  CHECK(space_contains(space_remaining, {expr}, std::identity{}));
  ...
  while (!space_remaining.empty()) {
    ...
    for (const int coeff : coeff_candidates) {
      ...
      if (space_contains(space_remaining, {candidate_expr}, std::identity{})) {
...
SpaceT space_basis(const SpaceT& space, const PrepareF& prepare) {
  ...
  for (const auto& expr : mapped(space, prepare)) {
    ret.push_back(expr);
    const int new_rank = space_rank(ret, std::identity{});
    if (new_rank == rank) {
      ret.pop_back();
    }
    rank = new_rank;
